...
public class PathFormat {
    ...
    private static String getString(String pattern) {
        ...
        if (pattern.indexOf("time") != -1) return getTimestamp();
        if (pattern.indexOf("yyyy") != -1)
            return getFullYear();
        if (pattern.indexOf("yy") != -1) return getYear();
        if (pattern.indexOf("mm") != -1) return getMonth();
        if (pattern.indexOf("dd") != -1)
            return getDay();
        if (pattern.indexOf("hh") != -1) return getHour();
        if (pattern.indexOf("ii") != -1) return getMinute();
        if (pattern.indexOf("ss") != -1) return getSecond();
